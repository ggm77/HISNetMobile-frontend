...
export type Block = { day: number; start: number; end: number; title: string; short?: string; room: string | null; meta: string | null; section: string | null }
...
export function flattenTimetable(tt: Timetable | null): Block[] {
  ...
  tt.courses.forEach((c) => {
    c.slots.forEach((s) => {
      ...
      if (day === -1) return
      blocks.push({ day, start: s.startPeriod, end: s.endPeriod, title: c.name, room: s.room, meta: c.professor, section: c.section })
    })
  })
  return blocks
}
